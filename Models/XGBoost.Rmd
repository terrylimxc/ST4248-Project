...
head(prediction)
conf <- table(predict=prediction, truth=test$IS_DIABETIC)
accuracy <- (conf[1,1]+conf[2,2]) / 49909
recall <- conf[2,2] / (conf[2,2] + conf[1,2])
precision <- conf[2,2] / (conf[2,2] + conf[2,1])
auc_score <- auc(test$IS_DIABETIC, prediction)
f1 <- 2*(precision*recall)/ (precision + recall)


xgb.importance(model = bstSparse)
xgb.plot.importance(xgb.importance(model= bstSparse))
```
